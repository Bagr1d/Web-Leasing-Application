...
@section('content')
    ...
    <form action="{{ route('contracts.update', $contract->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <select class="form-control" id="offer_id" name="offer_id" required>
                @foreach($offers as $offer)
                    <option value="{{ $offer->id }}" {{ $offer->id == $contract->offer_id ? 'selected' : '' }}>{{ $offer->offer_name }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control" id="total_cost" name="total_cost" value="{{ $contract->total_cost }}" required>
        </div>
